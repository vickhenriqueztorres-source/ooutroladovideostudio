...
const runDir = path.join(cwd, 'runs', 'diario-oficial-3-da-madrugada', runId);
const outDir = path.join(cwd, 'out');
const masterVideoPath = path.join(runDir, 'final_master.mp4');
const publicOutVideoPath = path.join(outDir, 'diario-oficial-3-da-madrugada.mp4');
const artifactOutVideoPath = path.join(artifactsDir, 'diario-oficial-3-da-madrugada.mp4');
const isolatedPublicDir = path.join(runDir, '.remotion-public-diario');

fs.mkdirSync(runDir, { recursive: true });
fs.mkdirSync(outDir, { recursive: true });
fs.mkdirSync(isolatedPublicDir, { recursive: true });

console.log('🎬 PREPARANDO RENDER DO MASTER DEFINITIVO: O Outro Lado das 3 da Madrugada...');

// 1. Isolated public dir
const requiredRoots = [
  'assets',
  'identity',
  path.join('episodes', 'diario-oficial-3-da-madrugada'),
  'audio'
];
...
const remotionCmd = `npx remotion render remotion/index.ts EpisodeDiarioOficial "${masterVideoPath}" --public-dir="${isolatedPublicDir}" --concurrency=4 --gl=angle`;

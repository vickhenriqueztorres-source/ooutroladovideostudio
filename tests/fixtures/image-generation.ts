...
const execPlan=(providerId="mock-image-provider",adapterVersion="1.0.0",promptIds:string[]=[])=>({providerId,adapterVersion,executionPlanId:"plan-1",jobs:promptIds.map((p,i)=>({jobId:`job-${i+1}`,promptId:p,inputArtifactIds:[`art-${i+1}`],providerPayloadRef:`payload-${i+1}`,idempotencyKey:`idem-${p}`,expectedAssetKind:"IMAGE",expectedAspectRatio:"16:9",expectedResolution:"1920x1080",requiredPostProcessing:[],preflightChecks:["reference-approved"],postflightChecks:["output-hash"],retryPolicy:"LIMITED"as const,failureResumePoint:"IMAGE_JOBS_CREATED"}))})

...
export interface LinhasegurapresidencialMicroStructureSC014SceneProps {
  accentColor?: string;
...
/**
 * 🔬 LinhasegurapresidencialMicroStructureSC014Scene: Reconstrução Isométrica 3D Documental
 * Desenvolvido autonomamente pelo Squad de Motion Graphics.
 * Arquétipo: MICROSCOPIC_STRUCTURE
 * Mecanismo: Se alguém tentar curvar um cabo ou perfurar um duto para grampear o feixe de luz, a perda microscópica de fótons dispara um alarme imediato.
 */
export const LinhasegurapresidencialMicroStructureSC014Scene: React.FC<LinhasegurapresidencialMicroStructureSC014SceneProps> = ({
  accentColor = '#FF5500',
  telemetryColor = '#00F0FF',
  headerTag = 'RAIO-X 3D // MICROSCOPIC STRUCTURE',
  title = 'ANÁLISE DE MICROESTRUTURA TÊXTIL // SC_014',
  subtitle = 'COMPOSIÇÃO DE POLPA: 100% FIBRAS NOBRES DE ALGODÃO | ABNT / CASA DA MOEDA',
  ...
  return (
    ...
    >
      ...
      <IsometricViewport
        rotateX={50 + cameraDrift}
        rotateZ={-45 + cameraDrift * 0.5}
        scale={1.15 * entrance}
        perspective={1400}
      >
        ...
        {/* Camada 1: POLIETILENO BLINDADO */}
        <IsometricCuboid
          key="LAYER_OUTER_SHEATH"
          width={540}
          height={320}
          depth={24}
          elevation={(160 + (0 * 28 * layerSplit))}
        ...
        {/* Camada 2: FIOS DE AÇO HELICOIDAIS */}
        <IsometricCuboid
          key="LAYER_ARMOR_STEEL"
          width={540}
          height={320}
          depth={24}
          elevation={(100 + (1 * 28 * layerSplit))}
          topColor="rgba(28, 33, 44, 0.85)"
          leftColor="rgba(18, 22, 30, 0.9)"
          rightColor="rgba(10, 12, 18, 0.95)"
          borderColor="rgba(255, 255, 255, 0.12)"
        ...
        {/* Camada 3: TUBO DE COBRE 10kV */}
        <IsometricCuboid
          key="LAYER_COPPER_TUBE"
          width={540}
          height={320}
          depth={24}
          elevation={(40 + (2 * 28 * layerSplit))}
        ...
        {/* Camada 4: FIBRAS ÓPTICAS DE SÍLICA PURA */}
        <IsometricCuboid
          key="LAYER_FIBER_CORE"
          width={540}
          height={320}
          depth={24}
          elevation={(0 + (3 * 28 * layerSplit))}
          topColor="rgba(255, 85, 0, 0.22)"
          leftColor="rgba(255, 85, 0, 0.15)"
          rightColor="rgba(255, 85, 0, 0.08)"
          borderColor="#FF5500"
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 30, fontWeight: 900, color: '#F4F4F0', marginTop: 4 }}>
            <span style={{ color: accentColor }}>100%</span> <span style={{ fontSize: 17, fontWeight: 500, color: '#A1A1AA' }}>FIBRAS NOBRES DE ALGODÃO</span>
          </div>
          <div style={{ fontSize: 14, fontWeight: 700, color: telemetryColor, marginTop: 4, letterSpacing: 1 }}>ABNT / CASA DA MOEDA</div>
        </div>
      </div>
    </AbsoluteFill>
  );
